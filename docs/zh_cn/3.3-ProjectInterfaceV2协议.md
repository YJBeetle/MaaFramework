...
- welcome `string | string[]` **💡 v2.10.2**  
  欢迎消息，在用户首次使用时弹窗显示，亦可作为公告使用。每个字符串支持文件路径、URL或直接文本，内容支持Markdown格式，并支持国际化（以`$`开头）。公告标题应写入Markdown内容中。  
  v2.10.2 起可使用字符串数组声明多条公告，Client 应按数组顺序展示。数组不能为空。  

  ```jsonc
  "welcome": [
      "$notice",
      "announcements/2026/update.md"
  ]
  ```

  Client 应记录完整的有序公告列表；公告增删、重排以及任一内容更新均视为内容更新，并应再次提示用户。旧版 Client 可能无法解析数组写法，需要兼容旧版 Client 时请继续使用单个字符串写法。
...
- controller `object[]`  
  ...
  - type `'Adb' | 'Win32' | 'MacOS' | 'IOS' | 'PlayCover' | 'Gamepad' | 'Linux'`  
    控制器类型，取值为 `Adb`、`Win32`（仅 Windows）、`MacOS`（仅 macOS）、`IOS`（仅 macOS，连真机
    iPhone）、`PlayCover`（仅 macOS）、`Gamepad`（仅 Windows）和 `Linux`（仅 Linux）。
  ...
  - ios `object`  
    `IOS` 控制器的具体配置（仅 macOS）。通过 Xcode 的 CoreDevice / DeviceHub 通道连接真机 iPhone，
    不需要越狱、不装额外宿主 App，USB 连接即可。详见 [控制方式说明](2.4-控制方式说明.md#ios-screencap)。

    - udid `string`  
      可选。目标设备的 UDID。留空表示"只连了一台就用它"；连了多台时会拒绝启动并列出候选。

    - screencap `string`  
      可选。截图方式，不提供则使用默认（两条路都给，快的优先、解不了自动降级）。
      取值为 `Stream`、`ScreenshotService`，可用 `|` 或 `,` 叠加，例如 `Stream|ScreenshotService`。
...
- option `record<string, object>`  
  ...
  - pipeline_override `pipeline`  
    ...
    上述占位符的替换结果为**单个整数**（虚拟按键码），适用于 `key` 为 _int_ 的按键动作，例如：
