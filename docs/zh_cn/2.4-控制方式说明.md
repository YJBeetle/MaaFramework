...
> - [ProjectInterface V2](3.3-ProjectInterfaceV2协议.md) 支持配置 Win32、MacOS、iOS、Linux 等控制器的 screencap/input 控制方式。Adb 控制器的 screencap/input 使用 `MaaToolkitAdbDeviceFind` 自动检测和选择最优方式，无需手动配置。
...
iOS 控制器通过 Xcode 的 CoreDevice / DeviceHub 通道连接真机 iPhone（USB 即可，不需要越狱、不需要在
手机上装任何 App、也不需要 Apple 开发者签名注入）。输入走设备自带的 HID 面，截图有下面两条路。

### iOS Screencap

> 参考 [MaaDef.h](https://github.com/MaaXYZ/MaaFramework/blob/main/include/MaaFramework/MaaDef.h) 中的 `MaaIOScreencapMethod`

可多选，按位或组合；ProjectInterface 里用 `|` 或 `,` 分隔名称。默认两条都给（快的优先，坏了自动降级）。

| 名称 | API 值 | 速度 | 兼容性 | 需权限 | 支持后台 | 说明 |
| --- | --- | --- | --- | --- | --- | --- |
| Stream | `1` | 很快 | 中 | 无 | 是 | 复用设备的媒体流，取最新一帧。iPhone 14,4 / iOS 27 实测中位 13ms/张 |
| ScreenshotService | `2` | 中 | 高 | 无 | 是 | 每次一条 `capturescreenshot` RPC。实测简单画面中位 147ms，复杂画面 228ms |

> [!NOTE]
>
> - **Stream 是视频，不是单帧请求**：它拿到的是设备实时编码的 HEVC 流里最新一帧，所以是有损的，
>   并且后台要常驻一个解码器。画面很复杂时（实测无边记画满白线的看板，单个 IDR 256278 字节）会超过
>   平台解码后端的长度前缀上限，那一整帧被丢弃；只给 `Stream` 时这会直接报失败而不是偷偷变慢。
> - **ScreenshotService 的开销会随画面复杂度涨**（147ms → 228ms），但不会失败，也不占后台。
> - 两条都给时，视频路解不出帧会当场降级到截图服务；泵在后台偶尔还会重试视频路，画面变简单了就
>   自动回到快路径。
