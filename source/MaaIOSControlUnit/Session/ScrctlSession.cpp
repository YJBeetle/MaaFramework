...
bool ScrctlSession::screencap(cv::Mat& image, std::string& err)
{
    ...
    // "现在最新的那一帧"而不是"我上次取走的那一帧"：设备会把整条会话在起流后约 20 秒
    // 结束掉，而会话死了之后设备上任何画面变化
    // 都不会再推过来（一条会话只有起流后约 20 秒的租期，喂画面和回 RTCP 都续不上）——
    // 于是泵手里那张"最新"的帧其实是拆流前那一刻的旧画面。拿"我上次
    // 看过的"当起点，这张旧帧只要比它新就会被当成新帧交出去（实测：截图耗时 3ms，内容
    // 与动作前逐像素一致），对自动化来说这是最坏的一种错，因为它返回真、看着是成功的。
    const uint64_t floor_serial = pump_->serial();
    // 会话还新鲜（租期未到、心跳还在）时 wake() 是空操作，所以每次截图都催得起。去掉
    // 这一句的话，"静置 20 秒 -> swipe -> 截图"实测会一直交出拆流前那张旧图。租期已过
